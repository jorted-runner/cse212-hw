...
public static class SetsAndMaps
{
    ...
    public static string[] FindPairs(string[] words)
    {
        ...
        foreach (string word in words) {
            ...
            if (wordSet.Contains(reversed) && !visited.Contains(word) && !visited.Contains(reversed) && reversed != word) {
                var toAdd = $"{word} & {reversed}";
                visited.Add(word);
                result.Add(toAdd);
            }
        }
    ...
    public static string[] EarthquakeDailySummary()
    {
        ...
        // 3. Return an array of these string descriptions.
            // Create a description string for each earthquake (place and magnitude)
        var summaries = featureCollection.Features
                            .Select(f => $"Place: {f.Properties.Place} - Mag - {f.Properties.Mag}")
                            .ToArray();
        return summaries;
    }
}